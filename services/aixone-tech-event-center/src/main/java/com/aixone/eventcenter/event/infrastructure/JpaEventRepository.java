...
public interface JpaEventRepository extends JpaRepository<Event, Long>, EventRepository {
    ...
    @Override
    @Query("SELECT e FROM Event e WHERE e.createdAt BETWEEN :startTime AND :endTime")
    List<Event> findByCreatedAtBetween(@Param("startTime") Instant startTime, @Param("endTime") Instant endTime);
    
    @Override
    @Query("SELECT e FROM Event e WHERE e.tenantId = :tenantId AND e.createdAt BETWEEN :startTime AND :endTime")
    List<Event> findByTenantIdAndCreatedAtBetween(@Param("tenantId") String tenantId, 
