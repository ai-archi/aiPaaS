...
- **个性化定制**：支持用户级、角色级、租户级的个性化定制（菜单顺序、快捷入口、仪表盘布局等）
...
- **性能**：个性化配置查询响应时间不超过100ms，支持高并发访问
...
- **个性化配置管理**：工作台服务只负责用户个性化配置的管理（菜单顺序、快捷入口、仪表盘布局等），不涉及菜单主数据和权限决策
- **职责边界清晰**：菜单主数据和权限相关功能由目录服务提供，前端直接调用目录服务获取菜单，调用工作台服务获取个性化配置
...
| 模块名称 | 模块职责 | 主要功能 |
|---------|---------|---------|
| **个性化配置模块** | 用户个性化定制 | 用户级个性化、角色级个性化、租户级个性化（菜单顺序、隐藏菜单等）
| **仪表盘模块** | 仪表盘管理和展示 | 仪表盘配置、组件管理、布局管理、数据可视化 |
| **快捷入口模块** | 快捷入口管理 | 快捷入口配置、排序（权限控制由目录服务提供，前端调用目录服务进行权限校验）
...
**注意**：菜单主数据实体（Menu）由目录服务（aixone-app-directory）管理，前端直接调用目录服务获取菜单。工作台服务只负责用户个性化配置的管理，不涉及菜单主数据和菜单聚合。
...
- 职责：管理用户个性化配置（菜单顺序、隐藏菜单、快捷入口、仪表盘布局、主题等）
- 方法：
  - getUserPersonalization(userId): 获取用户完整个性化配置
  - saveUserPersonalization(userId, config): 保存用户完整个性化配置
...
- 注意：
  - 菜单主数据完全由目录服务管理，前端直接调用目录服务获取菜单
  - 工作台服务只负责个性化配置的存储和管理，不涉及菜单主数据和权限决策
...
#### 2.3.1 前端菜单和个性化配置获取流程

**前端调用流程**：
```
1. 前端调用目录服务获取菜单：
   a. 前端调用目录服务 `POST /api/v1/menus/filter`，传入用户ID和租户ID
   b. 目录服务内部处理：获取菜单主数据、获取用户权限信息、执行权限决策、过滤不可见菜单
   c. 目录服务返回用户可见菜单（已过滤）

2. 前端调用工作台服务获取个性化配置：
   a. 前端调用工作台服务 `GET /api/v1/workbench/menus/custom`，传入用户ID
   b. 工作台服务从数据库查询用户菜单个性化配置（顺序、隐藏菜单等）
   c. 工作台服务返回个性化配置
...
sequenceDiagram
    ...
    Frontend->>Directory: POST /api/v1/menus/filter (userId, tenantId)
    Note over Directory: 获取菜单主数据<br/>获取用户权限<br/>执行权限决策<br/>过滤不可见菜单
    Directory->>Frontend: 返回可见菜单（已过滤）
    
    Frontend->>Workbench: GET /api/v1/workbench/menus/custom (userId)
    Workbench->>Database: 查询个性化配置
    Database->>Workbench: 返回配置
    Workbench->>Frontend: 返回个性化配置
    
    Note over Frontend: 应用个性化配置<br/>调整顺序、隐藏菜单等
    Frontend->>Frontend: 渲染最终个性化菜单
```
...
| user_id | UUID | 用户ID |
| tenant_id | UUID | 租户ID |
| menu_id | UUID | 菜单ID（引用目录服务的菜单ID） |
| config | JSONB | 个性化配置（顺序、快捷入口、隐藏等） |
| created_at | TIMESTAMP | 创建时间 |
| updated_at | TIMESTAMP | 更新时间 |

**注意**：此表仅存储用户对菜单的个性化配置（如顺序、隐藏、快捷入口等），不存储菜单主数据。菜单主数据由目录服务管理。
...
| tenant_id | UUID | 租户ID |
| entry_id | UUID | 快捷入口ID |
| menu_id | UUID | 关联菜单ID（引用目录服务的菜单ID） |
| name | VARCHAR | 快捷入口名称 |
| icon | VARCHAR | 图标 |
| order | INTEGER | 显示顺序 |
| config | JSONB | 快捷入口配置 |
...
**注意**：快捷入口关联的菜单ID来自目录服务，工作台服务不管理菜单主数据。
...
| 缓存键 | 数据类型 | TTL | 说明 |
|-------|---------|-----|------|
| workbench:menucustom:{userId} | Hash | 1小时 | 用户菜单个性化配置缓存 |
...
- `GET /api/v1/workbench/menus/custom`：获取用户菜单个性化配置
  - 返回：菜单顺序、隐藏菜单列表、快捷入口配置等
  - 注意：前端需单独调用目录服务获取菜单主数据
- `PUT /api/v1/workbench/menus/custom`：保存用户菜单个性化配置
  - 请求体：菜单顺序、隐藏菜单列表、快捷入口配置等
  - 注意：仅保存个性化配置，不涉及菜单主数据
...
**配置项**：
- menu_order: 菜单顺序（用户自定义的菜单显示顺序）
- hidden_menus: 隐藏的菜单列表（用户选择隐藏的菜单ID列表）
- quick_entries: 快捷入口配置（用户设置的快捷入口列表）
- custom_icons: 自定义图标（用户为菜单设置的自定义图标）
- custom_names: 自定义名称（用户为菜单设置的自定义名称）

**配置管理流程**：
1. 前端调用工作台服务 `GET /api/v1/workbench/menus/custom` 获取用户个性化配置
2. 工作台服务从数据库查询用户菜单个性化配置
3. 返回个性化配置给前端
4. 前端根据个性化配置调整菜单显示（顺序、隐藏、快捷入口等）

**注意**：
- 菜单主数据完全由目录服务管理，前端直接调用目录服务获取菜单
- 工作台服务只负责个性化配置的存储和管理，不涉及菜单主数据和权限决策
- 前端负责将目录服务返回的菜单与工作台服务返回的个性化配置进行合并
...
3. 前端应用个性化配置：
   - 根据menu_order调整菜单顺序
   - 根据hidden_menus隐藏指定菜单
   - 根据quick_entries添加快捷入口标识
   - 根据custom_icons和custom_names应用自定义样式
4. 前端渲染最终个性化菜单
```
...
**依赖服务**：aixone-app-directory（间接依赖，前端直接调用）
...
**前端调用流程**：
- 前端调用目录服务 `POST /api/v1/menus/filter` 获取用户可见菜单（已过滤）
- 前端调用工作台服务 `GET /api/v1/workbench/menus/custom` 获取用户菜单个性化配置
- 前端将菜单和个性化配置合并，渲染最终个性化菜单

**职责划分**：
- **目录服务**：负责菜单主数据管理、权限数据管理、权限决策、权限过滤，前端直接调用
- **工作台服务**：负责用户个性化配置的存储和管理，前端调用获取个性化配置

**缓存策略**：
- 个性化配置缓存1小时
- 配置变更时清除相关缓存
- 注意：菜单主数据和权限数据由目录服务管理，工作台服务不涉及
...
**权限控制职责划分**：
- **目录服务**：负责所有权限相关功能（权限数据管理、权限决策、权限校验、权限过滤），前端直接调用
- **工作台服务**：不涉及权限控制，只负责个性化配置管理

**权限控制方式**：
- **菜单权限**：前端直接调用目录服务进行菜单权限过滤，目录服务基于RBAC/ABAC模型进行权限决策
- **数据权限**：基于租户的数据权限控制（工作台服务层面，仅针对个性化配置数据）
- **API权限**：工作台服务的API权限校验由Gateway统一处理，通过目录服务进行权限校验

**权限校验流程**（工作台服务API）：
```
1. 前端请求工作台服务API
2. Gateway验证Token有效性（认证服务）
3. Gateway调用目录服务进行权限校验（目录服务）
4. 目录服务返回权限校验结果
5. Gateway根据权限校验结果决定是否允许访问工作台服务API
```
...
1. **个性化配置管理**：工作台服务只负责用户个性化配置的管理（菜单顺序、隐藏菜单、快捷入口、仪表盘布局等），不涉及菜单主数据和权限决策
2. **职责边界清晰**：菜单主数据和权限相关功能由目录服务提供，前端直接调用目录服务获取菜单，调用工作台服务获取个性化配置
...
| **工作台服务（axione-app-workbench）** | 工作台界面、个性化配置、管理功能 | ✅ 个性化配置管理（菜单顺序、隐藏菜单、快捷入口等）、仪表盘<br>✅ 提供认证服务的管理界面和API<br>❌ 不管理菜单主数据，不进行权限决策，不进行菜单聚合<br>✅ 前端直接调用目录服务获取菜单，调用工作台服务获取个性化配置 |
...
5. **职责边界**：严格遵守服务职责边界，菜单主数据和权限相关功能由目录服务提供（前端直接调用），认证相关功能由认证服务提供，工作台服务只负责个性化配置管理和认证服务管理功能提供
