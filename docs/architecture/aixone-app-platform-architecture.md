...
**协作关系**：
- **技术平台**：依赖 axione-tech-auth（身份认证、Token管理）
- **应用平台**：依赖 aixone-app-directory（菜单主数据、权限数据、权限过滤）
- **技术平台**：依赖 axione-tech-config（配置管理）
- **事件中心**：依赖 axione-event-center（事件订阅，接收消息事件）
- **智能平台**：依赖 axione-ai-knowledge（智能搜索、智能推荐）

**职责边界**：
- **工作台服务**：负责菜单聚合、个性化配置、仪表盘、快捷入口、消息聚合
- **工作台服务**：提供认证服务的管理界面和API（Token管理、客户端管理等）
- **工作台服务**：不进行权限决策，所有权限过滤和权限校验均调用目录服务
- **认证服务（axione-tech-auth）**：负责身份认证、Token管理，不管理权限
- **目录服务（aixone-app-directory）**：负责菜单主数据、权限数据管理、权限决策、权限过滤
...
**协作关系**：
- **技术平台**：依赖 axione-tech-auth（身份认证、Token管理）
- **应用平台**：依赖 aixone-app-directory（权限数据管理、权限决策、权限校验）
...
**协作关系**：
- **技术平台**：依赖 axione-tech-auth（身份认证、Token管理）
- **应用平台**：依赖 aixone-app-directory（应用权限控制、权限校验）
...
**性能优化**：
- 菜单聚合结果缓存，减少目录服务（aixone-app-directory）调用
- 个性化配置批量查询，减少数据库访问
- 使用Redis Cache-Aside模式，提升查询性能

**职责边界**：
- **菜单聚合服务**：负责调用目录服务获取菜单主数据，调用目录服务进行权限过滤
- **目录服务**：负责菜单主数据管理、权限数据管理、权限决策、权限过滤
- **工作台服务**：不进行权限决策，只负责聚合菜单数据和个性化配置
...
**依赖服务**：
- **axione-tech-auth**：认证服务
  - 用途：用户身份认证、Token管理（颁发、校验、刷新、黑名单）
  - 集成方式：通过Gateway统一认证、REST API调用（管理接口）
  - 场景：用户登录、Token验证、管理功能（Token管理、客户端管理等）
  - 职责边界：仅负责身份认证和Token管理，不管理权限，管理接口由工作台服务提供

- **aixone-app-directory**：目录服务
  - 用途：菜单主数据管理、权限数据管理、权限决策、权限过滤
  - 集成方式：REST API调用
  - 场景：菜单主数据拉取、用户权限获取、权限过滤、权限校验
  - 职责边界：负责所有权限相关功能，工作台服务调用目录服务进行权限过滤和权限校验
...
- **多租户认证**：支持租户级认证隔离
- **Token管理**：Token由认证服务管理，通过Gateway统一验证Token有效性

#### 5.1.2 权限控制

**权限控制职责划分**：
- **目录服务（aixone-app-directory）**：负责所有权限相关功能（权限数据管理、权限决策、权限校验、权限过滤）
- **工作台服务（axione-app-workbench）**：调用目录服务进行权限控制和权限过滤，不进行权限决策
- **认证服务（axione-tech-auth）**：不管理权限，仅负责身份认证和Token管理

**权限控制方式**：
- **权限模型**：基于角色的访问控制（RBAC），由目录服务提供
- **权限粒度**：功能级、数据级权限控制，由目录服务提供权限决策
- **权限继承**：支持权限的继承和委托，由目录服务提供
- **菜单权限**：调用目录服务进行菜单权限过滤
- **快捷入口权限**：调用目录服务进行快捷入口权限校验
- **API权限**：调用目录服务进行API权限校验
...
6. **职责边界清晰**：认证、权限、工作台服务职责明确，避免职责交叉
